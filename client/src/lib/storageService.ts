...
export async function updateEntry(id: string, updates: Partial<Entry>): Promise<Entry | null> {
  return apiFetch<Entry>(`/api/entries/${encodeURIComponent(id)}`, {
    method: 'PUT',
...
// Utilities
export async function exportAllData(): Promise<{ entries: Entry[]; goal: Goal }> {
  const [goal, today] = await Promise.all([
    getGoal(),
    getEntries(new Date().toISOString().slice(0, 10)),
  ]);
  return { entries: today, goal };
}
export async function clearAllData(): Promise<void> {
  console.warn('clearAllData() is not implemented in API mode');
}
export async function deleteEntriesForDate(date: string): Promise<number> {
